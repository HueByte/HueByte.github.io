...
- Menu: `src/components/Nav/Nav.tsx`. A dark clipped "corner blob" pinned top-left holds the hamburger and ~40 twinkling CSS sparks (deterministic positions from `src/lib/random.ts`); it opens a slide-in panel modelled on the Mirage menu. Entries come from `navLinks` in `src/lib/site.ts`. Mounted in `Layout.tsx`.
  - Boot screen: `index.html` carries `#boot` with its styles inline, so it paints from the document's first bytes rather than waiting for the bundle. `src/lib/boot.ts` fades it out once `DreamScene` reports its first rendered frame, keeping it up for at least 700 ms so a fast load still reads as an intro. A page claims the screen with `claimBootScreen()`; `startBootFailsafe()` in `main.tsx` dismisses it after the first paint for pages that never claim it, and after 8 s if a claim never resolves. Its background is a copy of the `.home` fallback, so the fade reveals a finished scene.
